...
namespace Clio.Demo.Core.Gateway
{
    public class SQLClient : ISQLClient
    {
        ...
        public async Task Execute(string query, string connectionString)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
        ...
        public async Task<T> ExecuteScalar<T>(string query, string connectionString) where T : class, IEntity
        {
            T result = default;
            try
            {
